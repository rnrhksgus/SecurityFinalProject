...
class WS:
    ...
    def uart_send_message(self, message):
        ...
        self.app.addListItem('list', message)
        if message == 'go':
            ...
            if (ll_on_off == 1):
                self.uart.uart_transmit('FRONT SIDE LAMP LEFT ON')
                ll_on_off = 0
            elif (ll_on_off == 0):
                self.uart.uart_transmit('FRONT SIDE LAMP LEFT OFF')
                ll_on_off = 1
        elif message == 'right':
            if (rl_on_off == 1):
            ...
            for i in motor_list:
                ...
                self.uart.uart_transmit(send_data)
